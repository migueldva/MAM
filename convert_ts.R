...
convert_ts <- function(ts_name,
  ...
  #  ts <- c(ts, xts(rep(0, len.aux), order.by = seq.Date(from = index(ts)[length(ts)] %m+% months(1), to = end.date, by = "month")))
  #} 
  
  #if(index(ts)[1] > start.date){
  #  len.aux <- length(seq(from = start.date, to = index(ts)[1] %m-% months(1), by = "month"))
  #  ts <- c(xts(rep(0, len.aux), order.by = seq.Date(from = start.date, to = index(ts)[1] %m-% months(1), by = "month")), ts)
  #} 
